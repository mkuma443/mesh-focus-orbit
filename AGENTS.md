...
- addon disable／reload／enable の前に、対象 `AddonPreferences` の既存プロパティ値を全て読み取り専用で snapshot する。reload／enable 後は、現在のproperty schemaに存在する互換プロパティだけを、型を確認して復元し、少なくとも `debug_display` と `retopoflow_target_island_filter` の値を明示的に確認して完了報告へ記載する。
- property schema が変わった場合は、存在しないプロパティや型不一致の値を推測して書き戻さない。既存設定がdefaultへ戻ったまま実環境テストを開始しない。
